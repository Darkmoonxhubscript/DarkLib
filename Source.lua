...
local function InsertTheme(Instance, Type)
  table.insert(darklib.Instances, {
    Instance = Instance,
    Type = Type
  })
  return Instance
...
local function VerifyTheme(Theme)
  for name,_ in pairs(darklib.Themes) do
    if name == Theme then
      return true
    end
  end
...
local function LoadSettings(FileName)
  if readfile and isfile and isfile(FileName) then
    ...
    if decode and typeof(decode) == "table" then
      if assert(decode, "UISize") then darklib.Save["UISize"] = decode["UISize"]end
      if assert(decode, "TabSize") then darklib.Save["TabSize"] = decode["TabSize"]end
      if assert(decode, "Theme") and VerifyTheme(decode["Theme"]) then darklib.Save["Theme"] = decode["Theme"]end
    end
  end
end;LoadSettings("dark library V5.lua")
...
local function AddEle(Name, Func)
  darklib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
  local Element = darklib.Elements[Ele](Instance, props, ...)
  return Element
end
...
function darklib:SetTheme(NewTheme)
  if not VerifyTheme(NewTheme) then return end
  
  darklib.Save.Theme = NewTheme
  SaveJson("dark library V5.lua", darklib.Save)
  Theme = darklib.Themes[NewTheme]
  
  table.foreach(darklib.Instances, function(_,Val)
...
function darklib:MakeWindow(Configs)
  local WTitle = Configs[1] or Configs.Name or Configs.Title or "dark Library V5"
  local WMiniText = Configs[2] or Configs.SubTitle or "by : dark9999"
  ...
  local UISizeX, UISizeY = unpack(darklib.Save.UISize)
  ...
  local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
    Size = UDim2.new(0, darklib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
  ...
  ConnectSave(ControlSize2, function()
    darklib.Save.TabSize = MainScroll.Size.X.Offset
    SaveJson("dark library V5.lua", darklib.Save)
  end)
  ...
  function Window:SelectTab(TabSelect)
    if type(TabSelect) == "number" then
      darklib.Tabs[TabSelect].func:Enable()
    else
      for _,Tab in pairs(darklib.Tabs) do
  ...
  function Window:MakeTab(paste, Configs)
    ...
    local function Tabs()
      ...
      table.foreach(darklib.Tabs, function(_,Tab)
    ...
    FirstTab = true
    local Tab = {}
    table.insert(darklib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
    Tab.Cont = Container
    ...
    function Tab:AddSection(Configs)
      ...
      local Section = {}
      table.insert(darklib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    function Tab:AddButton(Configs)
      ...
      local Button = {}
      table.insert(darklib.Options, {type = "Button", Name = BName, func = Button})
    ...
    function Tab:AddToggle(Configs)
      ...
      local Toggle = {}
      table.insert(darklib.Options, {type = "Toggle", Name = TName, func = Toggle})
    ...
    function Tab:AddDropdown(Configs)
      ...
      local Dropdown = {}
      table.insert(darklib.Options, {type = "Dropdown", Name = DName, func = Dropdown})
    ...
    function Tab:AddSlider(Configs)
      ...
      SliderIcon:GetPropertyChangedSignal("Position"):Connect(function()
        ...
        SaveSlider()
			...
			local Slider = {}
			table.insert(darklib.Options, {type = "Slider", Name = SName, func = Slider})
